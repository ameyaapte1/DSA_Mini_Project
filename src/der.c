/*
 *	DER library : Helper functions for RSA key file IO
...
/*This file contains function for storing and retrieving RSA keys stored in a Standard DER file format*/

unsigned int mpz_sizeof(const mpz_t num) {	/*get size of a mpz_int in bytes */
...
uint16_t read_16(int fd) {	/*Read 2 bytes of data */
	uint16_t i = 0;
	((uint8_t *) & i)[1] = read_8(fd);	/*endianess */
...
int write_16(uint8_t * der, uint16_t ui) {	/*write 2 bytes of data */
	write_8(der, ((uint8_t *) & ui)[1]);	/*endianess */
...
int write_RSAPrivateKey(uint8_t * der, RSAPrivateKey * key) {	/*Write a RSA key to a der file */
	uint8_t *buf;
	unsigned long size;
	int i;
	...
	buf = (uint8_t *) realloc(buf,sizeof(uint8_t) * mpz_sizeof(key->dp));
	mpz_export(buf, &size, 1, sizeof(uint8_t), 0, 0, key->dp);
	write_Length(der, size);
	for(i = 0; i < size; i++)
		write_8(der, buf[i]);

	buf = (uint8_t *) realloc(buf,sizeof(uint8_t) * mpz_sizeof(key->dq));
	mpz_export(buf, &size, 1, sizeof(uint8_t), 0, 0, key->dq);
	write_Length(der, size);
	for(i = 0; i < size; i++)
		write_8(der, buf[i]);

	buf = (uint8_t *) realloc(buf,sizeof(uint8_t) * mpz_sizeof(key->iq));
...
int write_RSAPublicKey(uint8_t * der, RSAPrivateKey * key) {	/*Create a public file from a Private file */
	uint8_t *buf;
	unsigned long size;
	int i;
...
int DER_to_RSAPrivateKey(char *filepath, RSAPrivateKey * key) {	/*Read a RSA Private key from a DER file */
	int fd, length, buf, i;
	uint8_t *value;
...
int DER_to_RSAPublicKey(char *filepath, RSAPrivateKey * key) {	/*Read a public key from a DER file */
	int fd, length, buf, i;
	uint8_t *value;
...
int RSAPrivateKey_to_RSAPublicKey(char *infile,int type, char *outfile) {	/*Generate a RSA Public key from a RSA private Key */
	RSAPrivateKey key;
	RSAPrivateKey_init(&key);
	type ? DER_to_RSAPrivateKey(infile, &key):PEM_to_RSAPrivateKey(infile, &key);
